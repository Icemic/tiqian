...
class ShapingEvidenceRecorderTest {
    private val testTrace = TestTraceRecorder("ShapingEvidenceRecorderTest")
    ...
    fun recordShapingEvidence() {
        testTrace.section("recordShapingEvidence")
        if (!recordMode) {
            assertTrue(!recordMode, "recording skipped in this run")
            return
        }
