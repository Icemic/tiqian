...
internal class ParagraphDpLineBreaker(
    ...
    /**
     * `CompressionVisibilityDiscount` (实验中, ADR 0041 遗留校准项): how visible
     * one px/gap of COMPRESSION is relative to one px/gap of stretch. 1.0 is the
     * v2 `CompressionFirstFill` setting — compression at par with stretch. The
     * probe shows par pricing makes the DP decline push-ins the production fill
     * pass takes unconditionally under `PushInFirst`, so the DP leaves visible
     * stretch where the lookahead leaves none. Lower values price 标点 glue
     * compression as the near-invisible absorber CLREQ「先挤压、后拉伸」treats it
     * as. This is a MODEL constant (visibility) — deliberately NOT
     * `lineAdjustmentCompressBias`, which is the ENGINE's direction knob.
     */
    private val compressionVisibility: Float = 1f,
) : LineBreaker {
    ...
    private fun edgeGeometry(context: DpContext, line: LineCandidate, isSegmentLast: Boolean, hyphenEnd: Boolean): EdgeGeometry {
        ...
        if (overflow > 0f) {
            ...
            val dComp = overflow / gaps * compressionVisibility
            return EdgeGeometry(
