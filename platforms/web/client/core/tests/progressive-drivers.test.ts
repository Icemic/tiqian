...
function saveGlobals(names: string[]): GlobalEntry[] {
...
type ComputedStyleSurface = {
  getPropertyValue(name: string): string;
};

function makeComputedStyle(values: MakeComputedStyleValues = {}): CSSStyleDeclaration {
  ...
  const style: ComputedStyleSurface = {
    getPropertyValue: (name: string): string => {
  ...
  for (const key of Object.keys(props)) {
    Object.defineProperty(style, key, { value: props[key], enumerable: true, writable: true, configurable: true });
  }
...
// The fake pool conforms to the product LayoutJobPool surface so it rides the
// coordination slot without a cast; _calls records the driver traffic.
interface FakeLayoutJobPool extends LayoutJobPool {
  _calls: FakeLayoutJobPoolCalls;
}

interface FakeLayoutJobPoolOverrides {
  startJob?(spec: LayoutJobSpec): void;
  cancelJob?(root: Element): void;
  jobKind?(root: Element): string | null;
}

function makeFakeLayoutJobPool(overrides: FakeLayoutJobPoolOverrides = {}): FakeLayoutJobPool {
...
type PoolRestoreFn = () => void;

// The drivers reach the pool through the coordination service, so the fake
// pool is installed there for the duration of one test. The coordination
// service types the slot readonly; the swap rides the CoordinationPoolSlot
// boundary view from the shared test types.
function installFakePool(pool: FakeLayoutJobPool): PoolRestoreFn {
...
interface WithEnvFn<T> {
  (pool: FakeLayoutJobPool): T;
}

interface FakeCustomEventInit {
  bubbles?: boolean;
  composed?: boolean;
  detail?: unknown;
}
...
interface FakeWindowView {
  innerHeight: number;
  getComputedStyle(element: Element, pseudo?: string | null): CSSStyleDeclaration;
}

function withEnv<T>(fn: WithEnvFn<T>, overrides: WithEnvOverrides = {}): T {
  const saved = saveGlobals(["window", "document", "getComputedStyle", "CustomEvent", "Node"]);
  const fakePoolOptions: FakeLayoutJobPoolOverrides = {
  ...
  try {
    ...
    const computed = (_el: Element, _pseudo?: string | null): CSSStyleDeclaration => makeComputedStyle(values);
    Object.defineProperty(globalThis, "getComputedStyle", { value: computed, writable: true, configurable: true });
    const fakeWindow: FakeWindowView = {
      innerHeight: 800,
    ...
    Object.defineProperty(globalThis, "CustomEvent", {
      value: class {
        ...
        composed: boolean;
        detail: unknown;
        constructor(type: string, init?: FakeCustomEventInit) {
          this.type = type;
...
// The paragraph host's box geometry seed the makeElement helpers mutate, and
// the full rectangle the overridden getBoundingClientRect reports.
interface ParagraphRectSeed {
  top: number;
  bottom: number;
  width: number;
}
...
interface AttributeWriteRecord {
  name: string;
  value: string;
}

// The driver-observable extras makeElement wires onto the fake element base.
interface ParagraphElementExtras {
  events: Event[];
  setAttributes: AttributeWriteRecord[];
  removedAttributes: string[];
  _rect: ParagraphRectSeed;
  getBoundingClientRect(): FakeRect;
  getClientRects(): FakeRect[];
}

type ParagraphElementFakeSide = FakeElement & ParagraphElementExtras;
type ParagraphElement = ParagraphElementFakeSide & Element;

interface ElementChildSeed {
  nodeType: number;
  textContent: string;
}

interface ElementMakeOptions {
  tagName?: string;
  text?: string;
  width?: number;
  childNodes?: ElementChildSeed[];
}

function makeElement(initialAttributes: Record<string, string> = {}, options: ElementMakeOptions = {}): ParagraphElement {
  const attrs = new Map(Object.entries(initialAttributes || {}));
  const setAttributes: AttributeWriteRecord[] = [];
  const removedAttributes: string[] = [];
  const styleProps = new Map<string, string>();
  const text = options.text ?? "hello world";
  const rect: ParagraphRectSeed = { top: 0, bottom: 100, width: options.width ?? 300 };
  ...
  // The geometry and query overrides are wired through the fake-side view:
  // their return shapes stay fake-DOM flavored and never face the Element
  // constituents of the intersection.
  const fakeSide: ParagraphElementFakeSide = element;
  const originalGetBCR = fakeSide.getBoundingClientRect.bind(fakeSide);
  fakeSide.getBoundingClientRect = function (): FakeRect {
    const r = element._rect || rect;
    return { top: r.top, bottom: r.bottom, width: r.width, height: r.bottom - r.top, left: 0, right: r.width };
  };
  fakeSide.getClientRects = function (): FakeRect[] {
    return [];
  };
  fakeSide.closest = function (): null {
    return null;
  };
  fakeSide.querySelectorAll = function (): FakeElement[] {
    return [];
  };
  fakeSide.querySelector = function (): null {
    return null;
  };
  return element;
}
...
function makeFixtureParagraphElement(text: string = "hello world"): FakeElement & Element {
  const element = new FakeElement("p") as FakeElement & Element;
  element.width = 320;
  element.appendChild(new FakeText(text));
  return element;
}

interface PipelineDocumentElementView {
  clientHeight: number;
}

type PipelineElementFactory = (tagName: string) => FakeElement;
type PipelineFragmentFactory = () => FakeFragment;
type PipelineRangeFactory = () => Range;

interface PipelineRangeView {
  selectNodeContents(node: Node): void;
  getClientRects(): DOMRectList;
}

interface PipelineDocumentView {
  documentElement: PipelineDocumentElementView;
  createElement: PipelineElementFactory;
  createDocumentFragment: PipelineFragmentFactory;
  createRange: PipelineRangeFactory;
  head: FakeElement;
}
...
function makePipelineDocument(): Document {
  const documentView: PipelineDocumentView = {
  ...
  const documentObject: unknown = documentView;
...
// Test-local narrowing of the product TrackedParagraph: the source keeps the
// fake element surface the assertions read.
interface ParagraphTracked extends Omit<TrackedParagraph, "source" | "lowered"> {
  source: FakeElement & Element;
  lowered: LoweredParagraph;
}

function makeParagraph(overrides: ParagraphOverrides = {}): ParagraphTracked {
  const source = overrides.source || makeElement();
  const lowered: LoweredParagraph = overrides.lowered || {
    text: "test",
    textStyle: {
...
// Locale-divergence probe: the relaxed lowered view the 5e span carries. The
// prepare step reads only the span locale for the divergence verdict, and the
// real LoweredParagraph is assignable to this view, so the assertion direction
// holds.
interface RelaxedLoweredSpanStyle {
  locale: string;
}

interface RelaxedLoweredSpan {
  start: number;
  end: number;
  style: RelaxedLoweredSpanStyle;
}

interface RelaxedLoweredParagraph extends Omit<LoweredParagraph, "spans"> {
  spans: RelaxedLoweredSpan[];
}
...
interface CanonicalOptionsCall {
  root: Element;
  options: EnhanceOptions;
}

interface ParagraphCandidatesCall {
  root: Element;
  selector: string;
}
...
interface ObservedContextCalls {
  ...
  resolveEngineOptionsFromCanonical: CanonicalOptionsCall[];
  paragraphCandidates: ParagraphCandidatesCall[];
...
interface ObservedContextOverrides {
  candidates?: Element[];
  stranded?: Element[];
}
...
function makeObservedContext(root: Element, overrides: ObservedContextOverrides = {}): ObservedContext {
...
test("1e. processItem sets stale when measure drifts and does not process", function () {
  ...
  withEnv((pool) => {
    ...
    const p1Source = p1.source as ParagraphElement;
    p1Source._rect.width = 600;
    spec.processItem!(0);
...
test("1f. onItemsFinished aggregates stale across all items", function () {
  ...
  withEnv((pool) => {
    ...
    const p2Source = p2.source as ParagraphElement;
    p2Source._rect.width = 600;
...
test("5e. relayout main path: onItemsFinished calls finish which ejects unsupported paragraphs", function () {
  ...
  const relaxedLowered: RelaxedLoweredParagraph = {
    text: "test",
    textStyle: {
      fontFamilies: [],
      fontSize: 19,
  ...
  const renderedP = makeParagraph({
    ...
    lowered: relaxedLowered as LoweredParagraph,
  });
...
// Event detail payloads the finishing reporting layer dispatches on the root
// element (progressive-drivers' summary and error details).
...
interface RelayoutReadyEventDetail {
  enhancedCount: number;
  runtimeEnhancedCount: number;
  snapshotCount: number;
  issueCount: number;
...
interface ProgressiveErrorEventDetail {
  kind: string;
  error: string;
  durationMs: number;
  maxSliceMs: number;
...
test("6a. finish: dispatches tiqian:ready with correct detail fields", function () {
  ...
  withEnv((pool) => {
    ...
    const d = (readyEvent as CustomEvent).detail as ReadyEventDetail;
...
test("6b. relayout finish: dispatches tiqian:relayout-ready with relayout: true", function () {
  ...
  withEnv((pool) => {
    ...
    const relayoutDetail = (relayoutReadyEvent as CustomEvent).detail as RelayoutReadyEventDetail;
    assert.equal(relayoutDetail.relayout, true);
    assert.equal(relayoutDetail.failed, false);
    assert.equal(relayoutDetail.error, null);
    assert.equal(relayoutDetail.snapshotCount, 3);
    assert.equal(relayoutDetail.stale, false);
  });
});
...
test("6c. fail: sets data-tiqian-relayout-error attribute, dispatches error and summary events", function () {
  ...
  withEnv((pool) => {
    ...
    const errorDetail = (errorEvent as CustomEvent).detail as ProgressiveErrorEventDetail;
    assert.equal(errorDetail.kind, "Relayout");
    assert.equal(errorDetail.error, "Something went wrong");
    ...
    const summaryDetail = (summaryEvent as CustomEvent).detail as RelayoutReadyEventDetail;
    assert.equal(summaryDetail.failed, true);
    assert.equal(summaryDetail.error, "Something went wrong");
  });
});
...
test("6d. fail: detail truncated to 512 chars", function () {
  ...
  withEnv((pool) => {
    ...
    assert.ok(errorEvent);
    const errorDetail = (errorEvent as CustomEvent).detail as ProgressiveErrorEventDetail;
    assert.equal(errorDetail.kind, "Enhance");
  });
});
