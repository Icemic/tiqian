...
//! `platforms/web/server/core/scripts/plan-parity-oracle.ts` one to one; the
...
fn parity_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../core/build/plan-parity")
}
        ...
        if std::env::var("TIQIAN_REQUIRE_PARITY_ORACLE").is_ok_and(|value| value == "1") {
            panic!(
                "TIQIAN_REQUIRE_PARITY_ORACLE=1 but no oracle dump at {}; \
                 run node scripts/plan-parity-oracle.ts in platforms/web/server",
                oracle_path.display()
            );
        }
        eprintln!(
            "skipped: no oracle dump at {}; run node scripts/plan-parity-oracle.ts in platforms/web/server to produce it",
            oracle_path.display()
        );
        return;
    };
