...
artifact、native backend 与验证 app 的 minSdk 为 23。API 29+ `SystemFonts` 和 API 31 glyph API
只作可选字体来源 / oracle，API 23–28 的生产宿主使用 file、`ByteArray` 或 asset catalog；能力报告
...
| 38 | — | Android API 23 native 字体后端：平台无关 replayable face contract；宿主 file / `ByteArray` / asset catalog；HarfBuzz cluster/glyph/advance/placement/features 与同 face FreeType metrics/ink/outline；`LayoutResult` shape-once replay；API 29+ `SystemFonts` 与 API 31 glyph adapter 仅作可选来源/oracle；Compose 不路由回旧 renderer | `AndroidNativeFontBackendTest`（CJK/Latin fallback、locl/halt、missing glyph、outline、长文）及其同字体 native/platform glyph、advance、ink、line range/width 对照 + 共享 Compose Demo 截图 | `:shaping:native-font:connectedDebugAndroidTest` on API 23/27/30/31/current + `:shaping:android-adapter:connectedDebugAndroidTest` on API 31/current + `:frontend:compose:compileAndroidMain` + `:demo:android:assembleDebug` + layout golden/report | wip (2026-08-05 工作树已验证：API 23/27/30 native 4/4；API 31/37 native 4/4 + platform 16/16；API 23/27/30/31 Demo 截图。5040 字 debug emulator 探针 API 27 约 15.96s、API 30 约 14.70s、API 31 约 0.54s；release 真机性能与共享底层桥接仍待后续) |
