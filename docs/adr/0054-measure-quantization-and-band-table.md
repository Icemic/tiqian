...
引擎收到的仍是 15.5/30 配置；契约校验拿 computed 17.05 与记录的 15.5 比对，每次
...
其绕过场景不再存在。0053 五表契约的段落级标量清单随本决定删除
`gridEnabled`。
...
- **换带补丁（`SparseBandPatch`）**。页面只保留一份 HTML（构建期烘入静态页的
  ...
  的契约校验，浏览器扩展一类的外部注入在此暴露；不匹配即 miss 走整段重建，与
  自身缺陷是两类事件。`AtomicParagraphDomSwap` 不删除，退为全量重建路径（初次水合即烘入
  HTML、内容变更、契约不匹配）。内容变更按失效合一处置（见下）：换新 DataView，
...
- **闲时预热**。回填的填充顺序从访问驱动改为闲时驱动：coordinator 以最低优先级
  ...
  执行单元为每段每带，段内不可抢占（0039 调度架构弱点留档的取消粒度），剩余预算低于近期
...
- 框架集成契约不变（ADR 0042）：宿主框架拥有根元素，引擎拥有根内子树；换带对
