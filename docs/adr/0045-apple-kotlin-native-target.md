...
`AppleNativeCompositionTarget`:给组合核心模块(`:core`、`:font`、`:shaping:api`、`:clreq`、`:linebreak`、`:layout`)加 `macosArm64` 目标(暂不加 iOS,但 API 预留 writing-mode 等扩展点)。
...
- **不加 `macosX64`**:Kotlin 2.3.20 已移除该目标。
...
- 代价(诚实记录):多一个平台的构建面与长期维护;暂不含 iOS(仅留扩展点)。
...
- `:core`、`:font`、`:shaping:api`、`:clreq`、`:linebreak`、`:layout` 均编译到 `macosArm64`;`:layout:compileKotlinMacosArm64` 成功。
